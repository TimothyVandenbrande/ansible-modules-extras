...
options:
    ...
    server:
        description:
            - The name/ip of your RHV-m/oVirt instance.
        default: "127.0.0.1"
        required: false
    port:
        description:
    ...
    cluster:
        description:
            - The rhv/ovirt cluster in which you want you VM to start.
        required: false
    datacenter:
        description:
            - The rhv/ovirt datacenter in which you want you VM to start.
        required: false
        default: "Default"
    state:
        description:
    ...
    osver:
        description:
            - The operationsystem option in RHV/oVirt.
        default: "rhel_6x64"
        required: false
    mempol:
        description:
...
# add a CD to the disk cd_drive
  ...
  args:
    ...
    password: "{{ rhv.admin.pass }}"
    state: 'cd'
    cd_drive: 'rhv-tools-setup.iso'

# new host deployment + host network configuration
  action: rhvm
  args:
    name: "ovirt_node007"
    password: "{{ rhvm.admin.pass }}"
    type: "host"
    state: present
    cluster: "rhvm01"
    ifaces:
...
RHV_TYPE_OPTS = ['server', 'desktop', 'host']
...
class RHVConn(object):
    ...
    def __init__(self, module):
        ...
        try:
            ...
            raise Exception("Failed to connect to RHV-M.")
                ...
                if self.conn.hosts.add(HOST):
                    ...
                    while (state != 'non_operational' and state != 'up'):
                        ...
                        if state == 'non_responsive':
                            setMsg('Failed to add host to RHVM')
                            setFailed()
                            return False
    ...
    def set_CD(self, vmname, cd_drive):
        VM = self.get_VM(vmname)
        try:
            CD = VM.cdroms.get(id="00000000-0000-0000-0000-000000000000")

            CD.set_file(params.File(id=cd_drive))
...
class RHV(object):
    ...
    def setCD(self, vmname, cd_drive):
        ...
        if cd_drive:
            ...
            return self.conn.set_CD(vmname, "")
...
def core(module):
    ...
    if state == 'ping':
        r.test()
        return RHV_SUCCESS, {"ping": "pong"}
    elif state == 'info':
        name = module.params.get('name')
        if not name:
            setMsg("`name` is a required argument.")
            return RHV_FAILED, msg
        vminfo = r.getVM(name)
        return RHV_SUCCESS, {'changed': changed, 'msg': msg, 'vm': vminfo}
    elif state == 'present':
        created = False
        name = module.params.get('name')
        if not name:
        ...
        if actiontype == 'host':
            ...
            if r.setHost(name, cluster, ifaces) is False:
                return RHV_FAILED, msg
            return RHV_SUCCESS, {'changed': changed, 'msg': msg}
